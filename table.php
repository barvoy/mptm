...
function tm_str_monthly($how_many_months, $how_much_per_month) {
	return "Toastmasters International: $how_many_months * $$how_much_per_month";
}
...
function make_order_items_array($cfg, $order) {
	...
	$mo_cost = $club_cfg->{'fees'}->{'tmi_monthly'};
	$amt = $order_memb_start_mo * $mo_cost;
	array_push($item_all, [ $item_num, tm_str_monthly($order_memb_start_mo, $mo_cost), $amt ]);
	$item_num += 1;
	...
	array_push($item_all, [ '<b>=</b>', '<b>' . tm_str_total() . '</b>', '<b>' . $total_amt ]);
	$item_num += 1;
